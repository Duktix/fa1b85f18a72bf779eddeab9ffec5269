...
jQuery.fn.extend({
	jCountdown:function(){
		var Flip = function(target){
			...
			this.frame = function(v){
				if(v==undefined){
					...
					this._target.children(".jcd_text").css("background-position", left+"px"+" "+top+"px");
				}
			}
		...
		var Metal = function(target){
			...
			this.frame = function(v){
				if(v==undefined){
					...
					this._target.children(".jcd_text").css("background-position", left+"px"+" "+top+"px").css("opacity",opacity);
					this._target.children(".jcd_cover").css("opacity",opacity);
				}
			}
		...
		var init = function(setting){
			...
			html += '<div class="jcd_jCountdown">';
				if(displayDay){
					var lastItem = (!displayHour && !displayMinute && !displaySecond) ? " jcd_lastItem" : "";
					html += '<div class="jcd_group jcd_day'+lastItem+'">';
						for(var i=0; i<dayTextNumber; i++){
						itemClass = " jcd_item"+(i+1);
						lastClass = i==(dayTextNumber-1) ? " jcd_lastItem" : "";
						html += '<div class="jcd_container'+itemClass+lastClass+'">';
							if(style=="slide" || style=="crystal" || style=="metal"){
								html += '<div class="jcd_cover"></div>';
							}
							html += '<div class="jcd_text"></div>';
						html += '</div>';
						}
						if(displayLabel){
						html += '<div class="jcd_label"></div>';
						}
					html += '</div>';
				}
				
				if(displayHour){
					var lastItem = (!displayMinute && !displaySecond) ? " jcd_lastItem" : "";
					html += '<div class="jcd_group jcd_hour'+lastItem+'">';
						for(var i=0; i<hourTextNumber; i++){
						itemClass = " jcd_item"+(i+1);
						lastClass = i==(hourTextNumber-1) ? " jcd_lastItem" : "";
						html += '<div class="jcd_container'+itemClass+lastClass+'">';
							if(style=="slide" || style=="crystal" || style=="metal"){
								html += '<div class="jcd_cover"></div>';
							}
				...
				if(displayMinute){
					var lastItem = (!displaySecond) ? " jcd_lastItem" : "";
					html += '<div class="jcd_group jcd_minute'+lastItem+'">';
					for(var i=0; i<minuteTextNumber; i++){
						itemClass = " jcd_item"+(i+1);
						lastClass = i==(minuteTextNumber-1) ? " jcd_lastItem" : "";
						html += '<div class="jcd_container'+itemClass+lastClass+'">';
							if(style=="slide" || style=="crystal" || style=="metal"){
								html += '<div class="jcd_cover"></div>';
							}
				...
				if(displaySecond){
					html += '<div class="jcd_group jcd_second jcd_lastItem">';
						for(var i=0; i<secondTextNumber; i++){
						itemClass = " jcd_item"+(i+1);
						lastClass = i==(secondTextNumber-1) ? " jcd_lastItem" : "";
						html += '<div class="jcd_container'+itemClass+lastClass+'">';
							if(style=="slide" || style=="crystal" || style=="metal"){
								html += '<div class="jcd_cover"></div>';
							}
			...
			var countdownObject = target.children(".jcd_jCountdown");

			countdownObject.addClass("jcd_"+style);
			countdownObject.addClass("jcd_"+color);
			
			countdownObject.children(".jcd_group").css("margin-right",textGroupSpace+"px");
			countdownObject.children(".jcd_group.jcd_lastItem").css("margin-right","0px");
			countdownObject.children(".jcd_group").children(".jcd_container").css("margin-right",textSpace+"px");
			countdownObject.children(".jcd_group").children(".jcd_container.jcd_lastItem").css("margin-right","0px");
			...
			if(reflection){
				if((browser.msie && browser.version<10)){
					...
					reflectionObject.addClass("jcd_reflection");
					if(displayLabel){
						reflectionObject.addClass("jcd_displayLabel");
					}
					...
					if(reflectionBlur!=0){
						reflectionObject.css("filter","blur("+reflectionBlur+"px)");
					}
			...
			var countdownContainer = jQuery('<div class="jcd_jCountdownContainer"></div>');
			countdownContainer.append(countdownObject);

			target.append(countdownContainer);
			
			if(width!=0){
				var countdownScaleObject = jQuery('<div class="jcd_jCountdownScale"></div>');
			...
			index = 1;
			selector = ".jcd_group.jcd_day>.jcd_container.jcd_item";
			...
			index = 1;
			selector = ".jcd_group.jcd_hour>.jcd_container.jcd_item";
			while(countdownObject.find(selector+index).length){
			...
			index = 1;
			selector = ".jcd_group.jcd_minute>.jcd_container.jcd_item";
			while(countdownObject.find(selector+index).length){
			...
			index = 1;
			selector = ".jcd_group.jcd_second>.jcd_container.jcd_item";
			while(countdownObject.find(selector+index).length){
